...
export const BlogPostView = () => {
  ...
  const category = post ? blogCategories.find(c => c.slug === post.category) : null;
      ...
      <article className="min-h-screen bg-gray-50 pb-20">
        ...
        <header className="bg-emerald-900 border-b border-emerald-800 text-white relative pt-20 pb-16 px-4">
          ...
          <div className="max-w-4xl mx-auto relative z-10">
            <Link to="/blog" className="inline-flex items-center text-emerald-300 hover:text-white font-medium mb-8 transition-colors">
            ...
            <div className="flex items-center gap-4 bg-emerald-800/40 p-4 rounded-xl border border-emerald-700/50 inline-flex">
              ...
              <div>
                <p className="text-sm text-emerald-100 font-medium">Por <strong className="text-white">Elvis Dias</strong></p>
                <p className="text-xs text-emerald-300">Jornalista Profissional (DRT 1466/RO)</p>
              </div>
            </div>
          </div>
        </header>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 mt-12">
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 md:p-12">
            ...
            <div className="bg-gray-50 border border-gray-100 rounded-2xl p-8 mb-12">
              <h3 className="text-lg font-bold text-gray-900 mb-4 tracking-tight uppercase">Índice do Conteúdo</h3>
              <ul className="space-y-3">
                {post.sections.map((sec, idx) => {
                  const anchorId = `sec-${idx}`;
                  return (
                    <li key={idx}>
                      <a href={`#${anchorId}`} className="text-emerald-700 font-medium hover:text-emerald-800 hover:underline flex items-start gap-2">
            ...
            <div className="prose prose-lg prose-emerald max-w-none">
              {post.sections.map((section, idx) => (
                <div key={idx} id={`sec-${idx}`} className="mb-12 scroll-mt-24">
                  ...
                  {section.hasAd && (
                    <AdSenseBlock slot={`mid-article-${idx}`} className="my-10" />
                  )}

                  {/* Contextual CTA */}
                  {section.hasCta && (
                    <div className="my-10 bg-emerald-50 border border-emerald-200 rounded-2xl p-8 shadow-sm">
                      <h4 className="text-xl font-bold text-emerald-900 mb-3">{section.hasCta.text}</h4>
                      <Link 
            ...
            <div className="mt-16 bg-white border border-gray-200 rounded-2xl p-8 flex flex-col md:flex-row items-center md:items-start gap-6 shadow-sm">
              ...
              <div className="text-center md:text-left">
                ...
                <p className="text-emerald-700 font-medium mb-3">Jornalista (DRT 1466/RO) e Especialista em Conteúdo Digital</p>
            ...
            {post.faqs && post.faqs.length > 0 && (
              <div className="mt-16 pt-12 border-t border-gray-100">
                <h2 className="text-3xl font-bold text-gray-900 mb-8 tracking-tight text-center md:text-left">Perguntas Frequentes</h2>
                <div className="space-y-6">
                  {post.faqs.map((faq, idx) => (
                    <div key={idx} className="bg-gray-50 border border-gray-100 rounded-2xl p-6 hover:border-emerald-200 transition-colors">
