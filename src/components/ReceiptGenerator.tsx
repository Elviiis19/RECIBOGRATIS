...
interface ReceiptData {
  ...
  // Novos campos para Cuidador
  cuidadorPaciente?: string;
  cuidadorDataInicio?: string;
...
export function ReceiptGenerator({
  ...
  useEffect(() => {
    if (title.toLowerCase().includes("cuidador")) {
      const parts = [];
      if (data.cuidadorPaciente)
        parts.push(
          `serviços de cuidador(a) prestados ao paciente ${data.cuidadorPaciente}`,
        );
      if (data.cuidadorDataInicio && data.cuidadorDataFim) {
        // Format dates if they are in YYYY-MM-DD
        ...
        parts.push(
          `no período de ${formatBR(data.cuidadorDataInicio)} a ${formatBR(data.cuidadorDataFim)}`,
        );
      }
      if (data.cuidadorPlantoes)
        parts.push(`correspondente a ${data.cuidadorPlantoes}`);
      if (data.cuidadorTransporte)
        parts.push(
          `incluindo R$ ${data.cuidadorTransporte} de auxílio transporte`,
        );
    ...
    data.cuidadorPaciente,
    data.cuidadorDataInicio,
    data.cuidadorDataFim,
    data.cuidadorPlantoes,
    data.cuidadorTransporte,
    title,
  ]);
  ...
  const getDocType = () => {
    ...
    if (t.includes("vale alimentação") || t.includes("vale-alimentação"))
  ...
  const isSemLogo =
    isReciboSimples ||
    (docType === "aluguel" && !title.toLowerCase().includes("logo")) ||
    docType === "promissoria";
  ...
  const steps =
    docType === "promissoria"
      ? [
          ...
          ...(isComAvalista
            ? [{ id: "avalista", title: "Dados do Avalista" }]
            : []),
  ...
  const handlePrint = useReactToPrint({
    ...
    onPrintError: (error) => {
      ...
      alert(
        "Não foi possível imprimir. Se você estiver no ambiente de visualização, abra o aplicativo em uma nova aba, ou use o botão de baixar PDF.",
      );
    },
  });
  ...
  const handleDocumentChange = (
    field: "pagadorDocumento" | "recebedorDocumento" | "avalistaDocumento",
    value: string,
  ) => {
  ...
  const validateCurrentStep = () => {
    ...
    if (stepId === "valor") {
      ...
      if (
        docType === "promissoria" &&
        (!data.quantidadeParcelas || Number(data.quantidadeParcelas) < 1)
      ) {
      ...
      if (
        data.recebedorDocumento &&
        !validateDocument(data.recebedorDocumento)
      ) {
  ...
  const handleWhatsApp = async () => {
    ...
    if (navigator.share) {
      try {
        ...
        if (blob) {
          const file = new File(
            [blob],
            `Recibo_${data.pagadorNome || "Documento"}.pdf`,
    ...
    alert(
      'Aviso: O WhatsApp Web (no computador) não permite enviar arquivos PDF diretamente pelo link. O sistema preencherá apenas o texto. Para enviar o documento, por favor, clique em "Baixar PDF" e anexe o arquivo manualmente na conversa.',
    );
          ...
          <>
            <p className="text-justify indent-8">
              ...
              <span className="font-bold">
                {data.data ? formatDate(data.data) : "___/___/20__"}
              </span>{" "}
              ...
              <span className="font-bold">
                {data.recebedorDocumento || "_________________________"}
              </span>{" "}
              ou à sua ordem, a quantia de{" "}
              <span className="font-bold">R$ {formatCurrency(data.valor)}</span>{" "}
              {data.valor && data.valor !== "0,00"
                ? `(${getValorExtenso(data.valor)})`
                : ""}
              , pagável em{" "}
              <span className="font-bold uppercase">
                {data.cidade || "_________________________"}
              </span>
              .
            </p>
            <p className="text-justify indent-8">
              Esta nota promissória é referente a{" "}
              <span className="font-bold uppercase">
                {data.referenteA ||
                  "________________________________________________________________________________________________"}
              </span>
              . Fica eleito o foro desta referida praça de pagamento para
        ...
        return (
          <>
            ...
            <p className="text-justify indent-8">
              ...
              , inscrito(a) no CPF/CNPJ sob o nº{" "}
              <span className="font-bold">
                {data.pagadorDocumento || "_________________________"}
              </span>
              .
            </p>
            <p className="text-justify indent-8 mt-2">
              Apresentamos o presente {getDocTitle()} no valor total de{" "}
              <span className="font-bold">R$ {formatCurrency(data.valor)}</span>{" "}
              {data.valor && data.valor !== "0,00"
                ? `(${getValorExtenso(data.valor)})`
                : ""}
            ...
            <p className="mt-4">
              <span className="font-bold">
                Forma de Pagamento sugerida/acordada:
              </span>{" "}
              {data.formaPagamento}{" "}
        ...
        return (
          <>
            <p className="text-justify indent-8">
              ...
              , o correspondente a vale-transporte / auxílio deslocamento no
              valor de{" "}
              <span className="font-bold">R$ {formatCurrency(data.valor)}</span>{" "}
              {data.valor && data.valor !== "0,00"
                ? `(${getValorExtenso(data.valor)})`
            ...
            <p className="text-justify indent-8">
              Declaro que os valores recebidos serão utilizados exclusivamente
              para despesas de deslocamento e assumo a responsabilidade por
              estas informações.
            </p>
          </>
        );
        ...
        return (
          <>
            <p className="text-justify indent-8">
              ...
              , o correspondente a vale-alimentação / auxílio alimentação no
              valor de{" "}
              <span className="font-bold">R$ {formatCurrency(data.valor)}</span>{" "}
              {data.valor && data.valor !== "0,00"
                ? `(${getValorExtenso(data.valor)})`
            ...
            <p className="text-justify indent-8">
              Declaro que os valores recebidos serão utilizados exclusivamente
              para despesas com alimentação e assumo a responsabilidade por
              estas informações.
            </p>
          </>
        );
          ...
          <>
            <p className="text-justify indent-8">
              ...
              {data.valor && data.valor !== "0,00"
                ? `(${getValorExtenso(data.valor)})`
                : ""}
              , referente ao pagamento do aluguel do imóvel localizado em:{" "}
              <span className="font-bold">
                {data.aluguelEndereco || "______________________"},{" "}
                {data.aluguelNumero || "___"}{" "}
                {data.aluguelComplemento ? ` - ${data.aluguelComplemento}` : ""}{" "}
                - {data.aluguelBairro || "______________"} -{" "}
                {data.aluguelCidade || "______________"}/
                {data.aluguelUf || "__"} - CEP:{" "}
                {data.aluguelCep || "________-___"} -{" "}
                {data.aluguelTipo || "_____________"}
              </span>
              , competência / mês de referência:{" "}
              <span className="font-bold uppercase">
                {data.aluguelMesRef || "_____"} / {data.aluguelAnoRef || "____"}
              </span>
              .
            </p>

            <p className="text-justify indent-8">
              Para maior clareza, firmo(amos) o presente documento para que
              produza os seus efeitos legais, dando plena, rasa e geral quitação
              pelo valor acima recebido.
            </p>
  ...
  const getPagadorLabel = () => {
    ...
    if (docType === "orcamento" || docType === "os" || docType === "termo")
      return "Nome do Cliente";
    if (docType === "vale" || docType === "vale-alimentacao")
      return "Nome da Empresa / Empregador (Quem paga)";
  ...
  const getRecebedorLabel = () => {
    ...
    if (docType === "orcamento" || docType === "os" || docType === "termo")
      return "Nome do Profissional ou Empresa";
    if (docType === "vale" || docType === "vale-alimentacao")
      return "Nome do Funcionário / Beneficiário";
  ...
  const getSignatureTitle = () => {
    ...
    if (docType === "orcamento" || docType === "os")
      return "Assinatura do Profissional / Empresa";
    if (docType === "vale" || docType === "vale-alimentacao")
  ...
  const getSignatureName = () => {
    if (docType === "promissoria")
      return data.pagadorNome || getSignatureTitle();
  ...
  const renderPromissoria = (index: number, total: number) => {
    ...
    return (
      <div
        key={`promissoria-${index}`}
        className="bg-[#FFFFE0] p-4 relative shadow-xl print:shadow-none border-2 border-black break-inside-avoid w-full mb-6 print:mb-4 font-sans"
      >
        ...
        <div
          className={`flex justify-between items-center border-b border-black pb-2 mb-3 ${isComAvalista ? "ml-10" : ""}`}
        >
          <div className="w-1/3">
            <p className="text-lg font-bold">{nTitulo}</p>
            <p className="font-semibold text-xs mt-1 uppercase">
              Vencimento:{" "}
              <span className="bg-white border text-sm border-black px-2 py-1 inline-block min-w-[120px] text-center ml-2">
                {currentVencimento}
              </span>
            </p>
          </div>
        ...
        <div
          className={`space-y-3 text-[13px] leading-relaxed text-black ${isComAvalista ? "ml-10" : ""}`}
        >
          <p className="text-justify indent-8 tracking-tight">
            ...
            pagarei(emos) por esta única via de NOTA PROMISSÓRIA a{" "}
            <span className="font-bold uppercase underline px-1">
              {data.recebedorNome ||
                "________________________________________________"}
            </span>
            , CPF/CNPJ{" "}
            ...
            , ou à sua ordem, a quantia de{" "}
            <span className="font-bold bg-gray-200 border border-gray-400 px-2 inline-block">
              {" "}
              {data.valor && data.valor !== "0,00"
                ? getValorExtenso(data.valor)
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 border-t border-black pt-2">
            <div>
              ...
              <div className="text-xs space-y-1">
                <p>
                  Nome:{" "}
                  <span className="uppercase border-b border-black border-dotted block w-full">
                    {data.pagadorNome || ""}
                  </span>
                </p>
                <div className="flex gap-2">
                  <p className="w-1/2">
                    CPF/CNPJ:{" "}
                    <span className="border-b border-black border-dotted block w-full">
                      {data.pagadorDocumento || ""}
                    </span>
                  </p>
                  <p className="w-1/2">
                    Data Emissão:{" "}
                ...
                <p>
                  Endereço:{" "}
                  <span className="uppercase border-b border-black border-dotted block w-full">
                    {data.devedorEndereco
                      ? `${data.devedorEndereco}, ${data.devedorNumero} ${data.devedorComplemento ? "- " + data.devedorComplemento : ""}`
                      : ""}
                  </span>
                </p>
              </div>
                ...
                <div className="text-xs space-y-1">
                  ...
                  <div className="flex gap-2">
                    <p className="w-1/2">
                      CPF/CNPJ:{" "}
                      <span className="border-b border-black border-dotted block w-full">
                        {data.avalistaDocumento || ""}
                      </span>
                    </p>
                    <p className="w-1/2">
                      Telefone:{" "}
                      <span className="border-b border-black border-dotted block w-full">
                        {data.avalistaTelefone || ""}
                      </span>
                    </p>
                  </div>
                  <p>
                    Endereço:{" "}
                    <span className="uppercase border-b border-black border-dotted block w-full"></span>
                  </p>
                </div>
              </div>
            ) : (
              ...
              <div className="flex gap-4 w-2/3 justify-end">
                <div className="flex flex-col items-center w-[200px]">
                  ...
                  <p className="text-[10px] font-bold uppercase">
                    {data.pagadorNome || "Assinatura Emitente"}
                  </p>
                </div>
                ...
                <div className="flex flex-col items-center w-[200px]">
                  <div className="w-full border-t border-black mb-1"></div>
                  <p className="text-[10px] font-bold uppercase">
                    {data.avalistaNome || "Assinatura Avalista"}
                  </p>
                </div>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start mt-8">
        ...
        <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100">
          ...
          <div className="min-h-[280px]">
                    ...
                    {(data.formaPagamento === "PIX" ||
                      ...
                      <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                        ...
                        >
                          ...
                          {data.formaPagamento.includes("Cartão") &&
                            "Bandeira e/ou 4 últimos dígitos (Opcional)"}
                          {data.formaPagamento === "Cheque" &&
                            "Número do Cheque / Banco (Opcional)"}
                        </label>
                        <input
            ...
            {(steps[currentStep].id === "pagador" ||
              steps[currentStep].id === "devedor") && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
                <div>
                  <label
                    htmlFor="pagadorNome"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleDocumentChange("pagadorDocumento", e.target.value)
                    }
                    placeholder="000.000.000-00"
                    maxLength={18}
                    className={cn(
                      "w-full px-4 py-3 text-lg border rounded-xl focus:ring-2 outline-none transition-all",
                      errors.pagadorDocumento
                        ? "border-red-500 focus:ring-red-500"
                        : "border-gray-300 focus:ring-emerald-500 focus:border-emerald-500",
                    )}
                  />
                ...
                {docType === "promissoria" && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in slide-in-from-top-2 duration-300">
                    ...
                    <div>
                      ...
                      <input
                        id="devedorEndereco"
                        name="devedorEndereco"
                        value={data.devedorEndereco}
                        onChange={handleChange}
                    ...
                    <div>
                      <label
                        htmlFor="devedorComplemento"
                        className="block text-sm font-semibold text-gray-900 mb-2"
                      >
                        Complemento
                      </label>
                      <input
                        id="devedorComplemento"
                        name="devedorComplemento"
                        value={data.devedorComplemento}
                        onChange={handleChange}
                        className="w-full px-4 py-3 text-lg border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500"
                      />
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <div>
                        ...
                        <input
                          id="devedorUf"
                          name="devedorUf"
                          value={data.devedorUf}
                          onChange={handleChange}
                          maxLength={2}
                          className="w-full px-4 py-3 text-lg border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 uppercase"
                        />
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <label className="flex-1 cursor-pointer">
                        <div className="w-full px-4 py-3 border-2 border-dashed border-gray-300 rounded-xl hover:border-emerald-500 hover:bg-emerald-50 transition-all flex items-center justify-center gap-2 text-gray-600">
                          ...
                          <span>
                            {data.logo ? "Trocar Logo" : "Fazer upload do Logo"}
                          </span>
                        </div>
                        <input
                          type="file"
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
                {docType === "aluguel" ? (
                  <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <input
                          id="aluguelEndereco"
                          name="aluguelEndereco"
                          value={data.aluguelEndereco}
                          onChange={handleChange}
                          placeholder="Rua, Avenida..."
                          className="w-full px-4 py-3 text-lg border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500"
                        />
                      ...
                      <div>
                        <label
                          htmlFor="aluguelComplemento"
                          className="block text-sm font-semibold text-gray-900 mb-2"
                        >
                          Complemento
                        </label>
                        <input
                          id="aluguelComplemento"
                          name="aluguelComplemento"
                          value={data.aluguelComplemento}
                          onChange={handleChange}
                          className="w-full px-4 py-3 text-lg border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500"
                        />
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="md:col-span-2">
                        ...
                        <input
                          ...
                          value={data.cuidadorPaciente}
                          onChange={handleChange}
                          placeholder="Nome de quem recebeu os cuidados"
                          className="w-full px-4 py-3 text-lg border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500"
                        />
                      </div>
                      <div>
                        <label
                          htmlFor="cuidadorDataInicio"
                          className="block text-sm font-semibold text-gray-900 mb-2"
                        >
                          Data Inicial
                        </label>
                        <input
                          id="cuidadorDataInicio"
                          type="date"
                          name="cuidadorDataInicio"
                          value={data.cuidadorDataInicio}
                          onChange={handleChange}
                          className="w-full px-4 py-3 text-lg border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500"
                        />
                      </div>
                      <div>
                        ...
                        <input
                          id="cuidadorDataFim"
                          type="date"
                          name="cuidadorDataFim"
                          value={data.cuidadorDataFim}
                          onChange={handleChange}
                          className="w-full px-4 py-3 text-lg border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500"
                        />
                      </div>
                      <div>
                        <label
                          htmlFor="cuidadorPlantoes"
                          className="block text-sm font-semibold text-gray-900 mb-2"
                        >
                          Quantidade de Plantões/Diárias
                        </label>
                        <input
                          id="cuidadorPlantoes"
                          name="cuidadorPlantoes"
                          value={data.cuidadorPlantoes}
                          onChange={handleChange}
                          placeholder="Ex: 5 plantões"
                          className="w-full px-4 py-3 text-lg border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500"
                        />
                      </div>
                      <div>
                        <label
                          htmlFor="cuidadorTransporte"
                          className="block text-sm font-semibold text-gray-900 mb-2"
                        >
                          Valor de Transporte (Se houver)
                        </label>
                        <input
                          id="cuidadorTransporte"
                          name="cuidadorTransporte"
                  ...
                  <div>
                    ...
                    >
                      {docType === "promissoria"
                        ? "Termos e acordos do pagamento"
                        : docType === "orcamento" || docType === "os"
                          ? "Descrição dos serviços orçados/executados"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    />
                    {errors.data && (
                      <p className="text-red-500 text-sm mt-2">{errors.data}</p>
                    )}
                  </div>
                </div>
                {docType === "promissoria" && (
            ...
            {steps[currentStep].id === "concluido" && (
              <div className="space-y-6 animate-in fade-in zoom-in-95 duration-500 text-center py-8">
                ...
                <p className="text-gray-600 mb-6 max-w-sm mx-auto">
                  Seu recibo foi gerado com sucesso. Revise a prévia ao lado e
        ...
        <div className="sticky top-24">
          <div className="bg-white p-2 rounded-2xl shadow-sm border border-gray-100 mb-4">
            <div className="bg-gray-50 rounded-xl p-4 flex justify-between items-center">
              ...
              <span className="text-xs bg-emerald-100 text-emerald-800 px-3 py-1.5 rounded-full font-bold tracking-wide uppercase">
                Padrão Jurídico Atualizado
              </span>
            </div>
          </div>
          ...
          <div className="bg-gray-100 p-4 rounded-2xl flex justify-center overflow-x-auto border border-gray-200">
            <div
              ref={componentRef}
              className="text-black font-sans print:w-full print:max-w-none"
              style={{
                width: "100%",
                maxWidth: "800px",
                margin: "0 auto",
            ...
            >
              {docType === "promissoria" ? (
                Array.from({
                  length: Number(data.quantidadeParcelas || 1),
                }).map((_, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className={isLastOnPage ? "print:break-after-page" : ""}
                    >
